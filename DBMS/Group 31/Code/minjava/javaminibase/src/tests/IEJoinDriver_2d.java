...
class IEJoinDriver_2d extends TestDriver implements GlobalConst {
	...
	public IEJoinDriver_2d(JoinsDriver jd){
	...
	private void ieJoin(int[] outputColumns, int[] predicateColumns, int[] operator, Heapfile heapR, Heapfile heapS){
		try {
			...
			// Initialization of BLOOM
			double chunk = 40.0;
			int bloomSize = (int) Math.ceil(n/chunk);
			int[] BLOOM = new int[bloomSize];
			double bloomData = 0.0;
			...
			for (i = 0; i<= m-1 && tupR !=null ; i++){
				...
				if (operator[0] == 2 && operator[1] == 2 ){
					for (j = off2; j<=n-1  ; j++){
						B[P_[j]] = 1;
						bloomData = (double)((P_[j]+1)/chunk);
						BLOOM[(int) Math.ceil(bloomData)-1]=1;
					}
				...
				if (operator[0] == 3 && operator[1] == 3){
					k_=off1 - eqOff;
					int bloomIndxEnd = (int) Math.ceil((double)((k_+1)/chunk))-1;

					//for(k = 0 ; k <= off1 - eqOff; k++){
					for(k = 0 ; k <bloomSize-1; k++){
						if( BLOOM[k] == 1){
							int start=(int)(k*chunk);
							int end = (int)((k+1)*chunk);
				...
				}else if (operator[0] == 3 && operator[1] == 2)
				{
					...
					{
						k_ = off1 + eqOff;
						int bloomIndx = (int) Math.ceil((double)((k_+1)/chunk))-1;

						for(k = bloomIndx; k <= bloomSize-1;k++){
							if( BLOOM[k] == 1)
							{
								int start=0,end=0;
								if (k == bloomIndx)
								{
									int innrLoop = (int)(k*chunk);  //4
									if(innrLoop==k_)
									{
										start=innrLoop;
										end = (int)(innrLoop+chunk)>=n?n:(int)(innrLoop+chunk);
									}
									else
									{
										start=k_;
								...
								{
									start = (int)(k * chunk);
									end = (int)(start+chunk)>=n?n:(int)(start+chunk);;
								}
