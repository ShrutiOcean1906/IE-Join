...
class BMDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test2 () {
	...
	SystemDefs.JavabaseBM.pinPage( lastPid, pg, /*emptyPage:*/ true );
      	...
      	// if given page does not exist in buffer frame it return negative frame value which throws
      	// bufmgr.ReplacerException
  ...
  protected boolean test4 () {
	    ...
	    if ( status == OK )
	    {
	      ...
	      if (status == OK)
	      {
	    	  try
	    	  {
	    		  //reduced the array size to avoud arrayindexoutofbound exception
