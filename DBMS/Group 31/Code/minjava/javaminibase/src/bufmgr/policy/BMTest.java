...
class BMDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test4 () {
	    ...
	    PageId firstPid = new PageId();
	    boolean status = OK;
	    SystemDefs sysdef = new SystemDefs( numpages, "LRUK" );
	    if ( status == OK )
	    {
	        System.out.print ("- Read the pages\n");
	        ...
	        try
	        {
	        	int pagenumber=frames[index];
	        	if(back(pagenumber,1)>MAX_SPACE)
	        	{
	        		SystemDefs.JavabaseBM.unpinPage( pagenumber, true );
	        	}
	        }
	        catch (Exception e) 
	        { 
	        	status = FAIL;
	        	System.err.print("*** Early Page Replacement " + pagenumber + "\n");
	            e.printStackTrace();
	        }
	        }
	      ...
	      if (status == OK)
	      {
	    	  try
	    	  {
	    		  ...
	    		  {
	    			  int pagenumber=frames[i];
	    			  if(HIST(pagenumber,1)>System.currentTimeMillis())
	    			  {
	    				  status=FAIL;
	    			  }
